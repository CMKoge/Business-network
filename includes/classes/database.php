...
class Database {
  ...
  # Update database
  public function update($table, $column, $attribute, $field = array()) {
    if (count($field)) {
      $keys = array_keys($field);
      $set = '';
      $pos = 1;
      ...
      $sql = "UPDATE {$table} SET {$set} WHERE {$column} = {$attribute}";
      if (!$this->query($sql, $field)->error()) {
        return true;
      }
    }
